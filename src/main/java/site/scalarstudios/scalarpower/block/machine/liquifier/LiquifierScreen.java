...
public class LiquifierScreen extends AbstractContainerScreen<LiquifierMenu> {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath("scalarpower", "textures/gui/liquifier.png");
    ...
    public LiquifierScreen(LiquifierMenu menu, Inventory inventory, Component title) {
    ...
    private void drawProgress(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        int progressPixels = Math.min(PROGRESS_WIDTH, Math.max(1, (int) ((float) PROGRESS_WIDTH * progress / cost)));
        graphics.blit(
    ...
    private void drawTankOverlay(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        graphics.fill(x + TANK_X + TANK_WIDTH - 1, y + TANK_Y, x + TANK_X + TANK_WIDTH, y + TANK_Y + TANK_HEIGHT, TANK_BORDER_WHITE);
