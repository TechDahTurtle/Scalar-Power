package site.scalarstudios.scalarpower.machines.generator.barometric;
...
public class BarometricGeneratorScreen extends AbstractContainerScreen<BarometricGeneratorMenu> {
    ...
    public BarometricGeneratorScreen(BarometricGeneratorMenu menu, Inventory inventory, Component title) {
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        String altitudeText = "Base: " + menu.getAltitudeSpu();
        String weatherText = menu.isBadWeather() ? "Weather: x1.5" : "Weather: x1.0";
        int yLevel = menu.getYLevel();
        int yFloor = (yLevel / 10) * 10;
        String yLevelText = "y = " + yLevel + " (Floor: " + yFloor + ")";

        int maxLineWidth = Math.max(this.font.width(generationText),
                Math.max(this.font.width(altitudeText),
                        Math.max(this.font.width(weatherText), this.font.width(yLevelText))));
        ...
        graphics.text(this.font, altitudeText, textX, firstLineY + lineStep, ENERGY_TEXT_COLOR, false);
        graphics.text(this.font, weatherText, textX, firstLineY + (lineStep * 2), ENERGY_TEXT_COLOR, false);
        graphics.text(this.font, yLevelText, textX, firstLineY + (lineStep * 3), ENERGY_TEXT_COLOR, false);
    }
